...
<title><s:text name="PUBLIC_ORDER_LIST"/></title>
...
function openDialog(dId) {
	$.get("/user/public-order-detail.htm", { id: dId }, function(data) {
		$('#'+dId).html(data);
	});
	$('#'+dId).dialog('open');
}
...
$(function() {
	<s:iterator value="publicOrderList">
		$('#<s:property value="orderPublicId"/>').dialog({
			autoOpen: false,
			show: 'fold',
			hide: 'fold',
			resizable: false,
...
<div>
	<span class="color_green bold"><s:text name="GROUP_COOKING_LESSONS"/></span> | 
...
<table cellspacing="0" cellpadding="2" width="100%" class="font_samll_2 order_table">
    <tr class="background_green bold">
    	<td colspan="8" align="center"><span><s:text name="GROUP_COOKING_LESSONS"/></span></td>
    </tr>
    <tr align="left">
        <td width="17%"><strong><s:text name="NO."/></strong></td>
        <td width="10%"><strong><s:text name="DATE"/></strong></td>
        <td width="12%"><strong><s:text name="TIME"/></strong></td>
        <td width="8%"><strong><s:text name="COURSE"/></strong></td>
    ...
    <s:iterator value="publicOrderList" status="s">
    	<tr class="<s:if test="#s.odd==true">background_green</s:if><s:else>background_white</s:else>">
	    	<td>
	    		<a class="hand" onclick="openDialog('<s:property value="orderPublicId"/>')"><strong><s:property value="orderPublicId"/></strong></a>
	    		<div id="<s:property value="orderPublicId"/>" title="<s:property value="orderPublicId"/>">
	            </div>
	    	</td>
	    	<td><s:date name="coursecalendar.classDate" format="yyyy-MM-dd"/></td>
	    	<td><s:property value="coursecalendar.classtime.classTimeContent"/></td>
	    	<td><s:property value="%{getText(coursecalendar.coursetrunktype.courseTrunkTypeNameKey)}"/></td>
	    	<td><a target="_blank" href="/others/kitchen.htm#kitchen_<s:property value="coursecalendar.courselocation.courseLocationId"/>"><s:property value="coursecalendar.courselocation.courseLocationName"/></a></td>
    ...
    <tr>
    	<td colspan="4" align="left">
    		<s:if test="publicOrderList.size() != 0">
    			<s:include value="../common/pages.jsp"></s:include>
    		</s:if>
    	</td>
        <td colspan="4" align="right">
        	<a href="<s:url action="public-orders-history" namespace="/user"/>"><s:text name="VIEW_MY_COOKING_HISTORY"/></a>
        </td>
    </tr>
</table>
